...
def z3_layer1_ws_net(x, w, b):
    fl_x = np.array([Real('fl_x%s' % i) for i in range(len(x))])
    ...
    for layer_index in range(len(candidates)):
        ...
        for neuron_index in range(len(candidates[layer_index])):
            ...
            if(layer_index == 0):
                ...
                y = z3_layer1_ws_net_adult(x, weight, bias)
...
# TODO: the layer_index goes upto max num of hidden layers (max hidden layers 9 = index up to 8)
def singular_verification_fairval(cand, df, weight, bias, ranges, pl_lb, pl_ub):
    ...
    for layer_index in range(len(candidates)):
        ...
        for neuron_index in range(len(candidates[layer_index])):
            ...
            # print(layer_index, neuron_index)
            if(layer_index == 0):
                x = np.array([Int('x%s' % i) for i in range(len(weight[layer_index]))])
                in_props = input_domain_constraint(df, x, ranges)
                ...
                x = np.array([Real('x%s' % i) for i in range(len(weight[layer_index]))])
                in_props = intermediate_domain_constraint(x, pl_lb, pl_ub, layer_index)
                y = z3_layer10_ws_net(x, weight, bias)
...
#    print('candi >>> ', candidates)
    ...
    s_dead_node_mask, s_candidates, s_compression = \
        singular_verification_adult(b_candidates, x_df, weight, bias, range_dict, pl_lb, pl_ub)
...
#    print('candi >>> ', candidates)
    ...
    ## Use verification on WS nodes, layer by layer, check always +/- and merge/remove
    s_dead_node_mask, s_candidates, s_compression = \
        singular_verification_fairval(b_candidates, x_df, weight, bias, range_dict, pl_lb, pl_ub)
    for l in s_dead_node_mask:
        if not 0 in l:
            l[0] = 0
