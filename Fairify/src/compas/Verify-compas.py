...
X = np.r_[X_train, X_val, X_test] # concatenate all X
num_features = len(X[0]) # columns
single_input = X_test[0].reshape(1, num_features)
...
print('Number of partitions: ', len(p_list)) # 120
shuffle(p_list)
...
for model_file in model_files:
    ...
    #TODO: these functions are model specific
    ...
    w = model_params[0:len(model_params):2]
    b = model_params[1:len(model_params):2]
    ...
    #    sd = s
        ...
        neuron_bounds, candidates, s_candidates, b_deads, s_deads, st_deads, pos_prob, sim_X_df  = \
            sound_prune_compas(df, w, b, simulation_size, layer_net, p) #TODO: change label_name inside function
        ...
        x = np.array([Int('x%s' % i) for i in range(num_features)])
        x_ = np.array([Int('x_%s' % i) for i in range(num_features)])
        ...
        for attr in A:
            if (attr in PA):
                in_props.extend(in_const_compas(df, x, attr, 'neq', x_)) #TODO: change label_name inside function
            else:
                in_props.extend(in_const_compas(df, x, attr, 'eq', x_))
    
        in_props.extend(in_const_domain_compas(df, x, x_, p, PA)) #TODO: and same here
